...
1. **禁止过度详细的身体描述**
   - ❌ 不应包含如 "私密处粉嫩未经人事"、"身体特征：B罩杯，胸型圆润挺翘，臀部小巧紧致" 等露骨描写
   - ✅ 仅包含必要的基本数据：身高、体重、胸/腰/臀围、罩杯等数字

2. **禁止预先生成性爱生理状态**
   - ❌ `性爱.乳房`、`性爱.小穴`、`性爱.肛门` 不应在初始化时出现
   - ✅ 这三个字段**仅在动情程度≥10 时才生成**

3. **禁止主观的性倾向描述**
   - ❌ 不应包含 "对角色扮演有特殊兴趣"、"兴奋时脸颊泛红" 等性反应推测
   - ✅ 初始性癖数组为空 `[]`，只在实际互动中追加

4. **禁止编造背景故事中的性经历**
   - ❌ 不应有 "初次经验尚未发生，对性充满好奇但也有些紧张"
   - ✅ 性经验次数为 0，无需补充说明
...
| -------- | -------------------------------------------- |
| 0-9      | 不生成 `乳房`、`小穴`、`肛门` 字段           |
| 10-39    | 可生成上述字段，描述应**简洁且非露骨**       |
| 40-69    | 可生成上述字段，描述应逐步**详细化**         |
| 70-100   | 可生成上述字段，允许**较详细的生理状态描写** |
