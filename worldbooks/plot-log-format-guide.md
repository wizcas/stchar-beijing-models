...
1. **第一步**：确认互动对象（是否正在与女模互动？）
2. **第二步**：更新出场女模列表（哪些女模当前在场？）
3. **第三步**：更新女模数据（女模属性是否需要更新？）
4. **第四步**：检查女模退场（是否有女模离开？）
5. **第五步**：检查拍摄任务（任务状态是否变化？）
6. **第六步**：更新 user 数据（玩家属性是否变化？）
7. **第七步**：生成推进选项（下一步有哪些选择？）
...
⚠️ **详细规则和检查清单请参考** `<status />` 或 `EXAMPLES.md`
...
- **完整 Schema 定义**：见 `status.md`
- **COT 思维链规则**：见 `COT.md`
- **快速参考版本**（< 1000 tokens）：见 `plot-log-quick.md`
